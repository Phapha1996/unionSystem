		...
		.onReady(function() {
			...
			var createWin = function(rowIndex, columnIndex) {
										...
										.confirm(
												...
												function(btn) {
													if (btn == 'yes') {
														...
														Ext.Ajax
																.request({
																	url : '../sys_pro_leader/userAction_pro_reply?dpmLV=4',
																	method : 'POST',
																	params : {
																		// 编码后作为参数发送到后台
																		replyContent : replyArea
																				.getValue(),
																		proId : ds
																				.getAt(
																						rowIndex)
										...
										.confirm(
												...
												function(btn) {
													if (btn == 'yes') {
														Ext.Ajax
																.request({
																	url : '../sys_pro_leader/userAction_pro_updateProposal?dpmLV=4',
																	method : 'POST',
																	params : {
																		// 编码后作为参数发送到后台
																		mdfAdv : mdfAdv
				...
				var replyArea = new top.Ext.form.TextArea({
					...
					readOnly : true,
					allowBlank : false,
					fieldLabel : '部门回复内容(待审核)'
				});
				var mdfAdv = new top.Ext.form.TextArea({
					width : '80%',
					height : 160,
					id : 'mdfAdv',
					name : 'mdfAdv',
					allowBlank : false,
					fieldLabel : '<font color="#0000FF">修改建议</font>'
				});
				var form = new top.Ext.form.FormPanel({
					//renderTo : Ext.get("pro_trock_grid"),
					frame : true,
					autoScroll:true,
					url : '../sys_pro_leader/userAction_pro_submitProposal',
					width : 800,
					html : '<div id="msg" class="tipmsg"></div>',
					height : 500,
					items : [ chartingRecord, changeopinion,replyArea,mdfAdv ],
					buttonAlign : 'center',
					labelAlign : 'right',
					buttons : [ sure, cancel ]
				});
						...
						proxy : new Ext.data.HttpProxy(
								{
									...
									// 对应1->2->3->4
									url : '../sys_pro_leader/userAction_pro_proposalTracing?name='
										+ encodeURI(encodeURI(mbName_pro)) + '&trackWay=leader'
								}),
						...
						columns : [
								...
								sm,
								{
									...
									renderer : function showbutton(value,cellmeta, record, rowIdx, colIdx, store, view) {
										var returnStr = "<INPUT type='button' value='审核回复' style='color:blue' onClick='myClick()'>";
										if (store.getAt(rowIdx).get('dpmReplyFlag') == 'new_reply') {
											returnStr = "<INPUT type='button' value='审核回复' style='color:blue;background:url(../img/newtip.png) no-repeat right top' onClick='myClick()'>";
											}
										return returnStr;
									}
								},{
			...
			var grid = new Ext.grid.EditorGridPanel({
				...
				listeners : {
					cellClick : function(grid, rowIndex, columnIndex, e) {
						...
						if (columnIndex == colCount - 19) {
							if (ds.getAt(rowIndex).get('dpmReply') != '') {
								createWin(rowIndex, columnIndex);
						} else {
								parent.Ext.Msg.alert("tips", "暂无回复内容");
							}
							// 弹出回复窗口
							 //createWin(rowIndex, columnIndex);
						}
						if (columnIndex == colCount - 20){
