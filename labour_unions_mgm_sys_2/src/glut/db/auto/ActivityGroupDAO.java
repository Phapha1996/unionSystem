 ...
 * 
 * @see glut.db.auto.ActivityGroup
 * @author MyEclipse Persistence Tools
 */
@Transactional
public class ActivityGroupDAO {
	private static final Logger log = LoggerFactory
			.getLogger(ActivityGroupDAO.class);
	// property constants
	...
	public ActivityGroup findById(java.lang.Integer id) {
		log.debug("getting ActivityGroup instance with id: " + id);
		try {
			ActivityGroup instance = (ActivityGroup) getCurrentSession().get(
					"glut.db.auto.ActivityGroup", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(ActivityGroup instance) {
		...
		try {
			List results = getCurrentSession()
					.createCriteria("glut.db.auto.ActivityGroup")
	...
	public List findByApplyLimit(Object applyLimit) {
		return findByProperty(APPLY_LIMIT, applyLimit);
	}
	...
	public ActivityGroup merge(ActivityGroup detachedInstance) {
		...
		try {
			ActivityGroup result = (ActivityGroup) getCurrentSession().merge(
					detachedInstance);
	...
	public static ActivityGroupDAO getFromApplicationContext(
			ApplicationContext ctx) {
		return (ActivityGroupDAO) ctx.getBean("ActivityGroupDAO");
	}
}