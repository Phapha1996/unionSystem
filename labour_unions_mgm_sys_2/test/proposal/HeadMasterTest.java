...
public class HeadMasterTest extends MyTransactionalSpringContextTests{
	@Resource
	...
	public void testUpdateProposal() {
		...
		String LdOpinionFlag = Constants.NEWREPLY_PPS_REPLY;//领导回复标志
		String ldOpinion = "ldOpinion";//
		try {
			...
			//校领导的
			dpmLv = "4";
			obj.setLdOpinion(ldOpinion);
			proposals.clear();
			proposals.add(obj);
			userServiceImp_pro.updateProposal(proposals, dpmLv, mdfAdv, proId);
			result4 = userDAO.findProposals("submitterName='蒋存波' and ld_opinion_flag='"+LdOpinionFlag+"'");
		} catch (Exception e) {
		...
		assertNotNull(result4.isEmpty() ? null : result4.get(0));
	}
